...
-(void)addSubviews{
    ...
    for (int i=0; i<3; i++) {
        ...
        text.textAlignment=NSTextAlignmentLeft;
        text.text=[subArray1 objectAtIndex:i];
        text.font=[UIFont systemFontOfSize:15];
...
-(void)addFuJian{
    ...
    bgView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, Height/17*6+Height/8*3-Height/16, Width, Height-44-(Height/17*6+Height/8*3))];
...
-(void)addPicture{
    ...
    if (messageattachment==1||messageattachment==2||messageattachment==3) {
        ...
        NSString *url=[NSString stringWithFormat:@"%@%@?fileId=%@",kDemoBaseURL,kGetFileURL,[[[listDic objectForKey:@"filelist"]objectAtIndex:0]objectForKey:@"id"]];
        UIImageView*imageView=[[UIImageView alloc]initWithFrame:CGRectMake(20, 140, 60, 60)];
