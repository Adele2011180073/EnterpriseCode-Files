...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *imageArray=@[@"\U0000e627",@"\U0000e656",@"\U0000e641",@"\U0000e63f",@"\U0000e683",@"\U0000e664"];
    NSArray *titleArray=@[@"部门",@"职位",@"密码修改",@"缓存清理",@"系统版本"];
    ...
    if (indexPath.section==1&&indexPath.row==0) {
        ...
        subTitle.text=[NSString stringWithFormat:@"v%@",appCurVersion];
    }
...
{
    //    //clear pic mov and wav
    NSFileManager *fileManager = [NSFileManager defaultManager];
        // get caches directory
        NSArray *cache = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        NSString *cachePath = [cache objectAtIndex:0] ;
        NSDirectoryEnumerator *direnum = [fileManager enumeratorAtPath:cachePath ]; //traverse directory
        NSString *fileName;
